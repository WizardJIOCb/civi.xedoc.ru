  ...
  return <section className="panel decision-panel">
    ...
    <div className="decision-list">
      {decisions.slice(0, 5).map((decision, index) => {
        ...
        const observer = decision.source === 'observer';
        return <article className="decision-card" key={decision.id} style={{ '--delay': `${index * 60}ms` } as React.CSSProperties}>
          <div className="decision-meta"><span className="decision-model">{observer ? <Eye size={12} /> : <Bot size={12} />} {observer ? 'Наблюдатель' : decision.model.split('/').at(-1)}</span><time>{relativeTick(decision.tick, tick)}</time></div>
